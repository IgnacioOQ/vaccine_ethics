...
class FullAgent:
    ...
    - Pass a per-simulation RNG via `rng` (e.g., Simulation's `random.Random(seed)`).
    - If `rng` is None, falls back to the global `random` module.
    """
    ...
    def __init__(self, x, y, grid_size=20, infection_prob=0.3, recovery_time=10,
        ...
        - viral_age_effect: Reduction per unit of viral age.
        - immune_adaptation_effect: Reduction per unit of immunity level.
        - rng: Random generator to use (e.g., `random.Random(seed)`). If None, use global `random`.
        """
        self.x = x
        self.y = y
        self.state = state
        ...
        # Per-agent RNG handle (module-compatible: supports .random/.randint/.choice/.sample)
    ...
    def move(self):
        ...
        move = self.rng.choice(['up', 'down', 'left', 'right'])
        ...
        if self.state == 'S':
            ...
            for agent in self.rng.sample(agents, len(agents)):
                if agent.x == self.x and agent.y == self.y and agent.state == 'I':
                    ...
                    # Further reduce infection probability based on immunity
                    infection_prob = max(
                        base_infection_prob *
            ...
            if self.rng.random() < base_death_prob:
                ...
                # Recover if infection time exceeds randomized threshold
                # This is very much a rectilinear activation function
                low = int(base_recovery_time // 2)
                high = int(base_recovery_time) if int(base_recovery_time) >= low else low + 1
                if self.time_infected >= self.rng.randint(low, high):
                    self.immunity_level += 1  # Gain some immunity
                    self.state = 'S'  # Become susceptible again (not truly recovered)
...
class FullAgent2:
    def __init__(self, x, y, grid_size=20, infection_prob=0.3, recovery_time=10,
                 death_prob=0.01, vul_penalty=0.5, vul_type='high', vaxxed=False,
                 state='S', vax_effect=0.7, viral_age_effect=0.05, immune_adaptation_effect=0.05,
                 rng=None, node_id=0):
        self.x = x
        self.y = y
        self.state = state
    ...
    def migrate_to(self, new_node_id, grid_size):
        """Move this agent to another node and re-seat on that node’s grid."""
        self.node_id = new_node_id
        # Option A: keep local (x,y) — only valid if grids are “aligned”
        # Option B (safer): re-seat randomly inside the new node’s grid:
        self.x = self.rng.randint(0, grid_size - 1)
        self.y = self.rng.randint(0, grid_size - 1)