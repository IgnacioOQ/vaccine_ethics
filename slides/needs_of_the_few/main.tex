...
Vaccination campaigns have had spectacular successes --- smallpox, polio, tetanus --- through \emph{herd immunity}.

\strut
\pause

This success underwrote a strong ethical claim during COVID-19:
\begin{itemize}
...
There exist COVID-like regimes in which vaccinating \emph{only the vulnerable} produces \textbf{fewer total deaths} than vaccinating \emph{everyone} ---

\strut
\pause
\dots even when the vaccine is
\begin{itemize}
...
Agents on a $25\times25$ grid follow a random walk. Contact between an infected and a susceptible agent transmits the virus with some probability.

\strut
\begin{center}
\resizebox{0.85\textwidth}{!}{%
\begin{tikzpicture}[node distance=1.6cm and 2.2cm]
  \node[block, fill=melon] (S) {\textbf{S}usceptible};
  \node[block, right=of S] (I) {\textbf{I}nfected};
  \node[block, fill=lightgray, above right=0.8cm and 1.8cm of I] (D) {\textbf{D}ead};
  \path[line] (S) -- node[above]{infection} (I);
  \path[line] (I) to[bend right=20] node[below]{recovery $+$ immunity} (S);
  \path[line] (I) -- node[above right]{death} (D);
\end{tikzpicture}}
\end{center}

\pause
\small
Susceptible $\to$ Infected $\to$ \{recover (back to S, with immunity) \emph{or} die\}.
\end{frame}
...
\strut
\pause
\textbf{\textit{The remedy is epistemic humility + honest uncertainty communication --- itself a contribution to pandemic preparedness.}}
\end{frame}
