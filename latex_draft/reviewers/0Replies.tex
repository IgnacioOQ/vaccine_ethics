...
% --- reply-letter macros ---------------------------------------------------
% \reply{}  our text to the reviewer. Renders plain: this letter is what the
%           reviewers receive, so it carries none of the colour conventions we
%           use in the working documents. Kept as a macro so the authorial
%           voice stays marked up in the source (and can be tinted again for
%           an internal read-through by editing this one line).
% \open{}   a placeholder for an item still being drafted; deliberately loud,
%           so that no unfinished reply can be shipped by accident. Must be
...
the manuscript. All section numbers refer to the revised manuscript, so that
each item can be located directly.
...
R1 $\cdot$ C5 & Specify which notion of herd immunity is in use & \S3.3\\\hline
...
mechanism of Luyten et al.\ is retained at the end of the same paragraph.} \ignacio{To be honest I would not use the expression 'negative heard immunity', I think its more confusing than clarifying.}
...
\rlabel{Response.}
\reply{We are grateful for this question, which turned out to be more than a
matter of terminology. The reviewer is right that the choice bears on an
epidemiological assessment of the model, and the manuscript had not made it.

The answer is settled by the structure of the model rather than chosen by us.
Our SISD model has no removed compartment. Agents who recover return to the
susceptible pool carrying partial immunity, which lowers their probability of
reinfection and of death without ever eliminating either. Since the
susceptible pool is continuously replenished and immunity is never sterilising,
there is no threshold whose crossing would bring transmission to an end. What
the population reaches is the first of the two notions the reviewer
distinguishes: a dynamic equilibrium at which the pathogen remains endemic.
The manuscript in fact already displayed this without saying so, in the example
run of the appendix, where the epidemic settles into an endemic but relatively
harmless state. We have now stated it where the reviewer looked for it.

One divergence from the models usually associated with that notion is worth
recording. There, recurrent waves are sustained by antigenic escape, as new
variants restore virulence and evade existing immunity. In our model the
pathogen instead attenuates with every transmission, so the equilibrium it
approaches is not merely endemic but progressively benign. We have made this
explicit as well, so that our appeal to a dynamic equilibrium is not read as a
claim about antigenic drift.}

\rlabel{Changes.}
\reply{In \S3.3 (\emph{Interpretation}), a new paragraph now precedes the
discussion of the negative herd immunity effect: ``Herd immunity is used in
more than one sense, and our model settles which one applies. There is no
removed compartment: agents who recover return to the susceptible pool carrying
partial immunity, which lowers their risk of reinfection without ever
eliminating it. No threshold is crossed after which transmission ceases. The
population instead reaches a dynamic equilibrium in which the pathogen remains
endemic at a low level, as the example run in Figure~6 illustrates. It is herd
immunity in this sense that we appeal to here. Unlike models in which new
variants restore virulence, the pathogen in our model attenuates with each
transmission, so the equilibrium it approaches is not merely endemic but
...
\rlabel{Response.}
\reply{We are grateful for this objection. It is the most serious challenge the
manuscript received, and the three counter-examples are well chosen.

We should say first what we are not disputing. We do not claim that virulence
decline was the dominant factor in the cases the reviewer names, and we accept
that in each of them low population immunity was the larger effect. We also
accept that the magnitude of intrinsic virulence decline in real pandemics is
contested. The manuscript now says so.

What our argument requires is weaker than what the objection denies. The claim
we examine is that mass vaccination would surely improve public health. That is
a claim of necessity, and a single coherent counter-example is enough to defeat
it. Our model is offered as a proof of concept in exactly that sense. It does
not predict what any pandemic will do. It exhibits a mechanism, places it in a
model no less reasonable than others in use, and shows that the ethical verdict
can reverse. For this the transmission--virulence trade-off need not be
typical, or even common. It needs to be real and of uncertain magnitude. The
reviewer's own formulation grants the first, allowing that there might be
evidence for effects of this kind, and disputes the second. A dispute about
magnitude is precisely the kind of uncertainty this paper is about.

This is why the objection, granted in full, does not weaken our conclusion.
Suppose virulence decline really is dwarfed by acquired immunity in most
pandemics. Then the models that guided policy encoded one particular weighting
of two mechanisms whose relative size was contested at the time and remains
contested now. A different weighting yields a different verdict about
free-riding and about who ought to be vaccinated. We do not claim that our
weighting is the right one. We claim that the verdict is sensitive to it, and
that this sensitivity is a reason for humility about the moral conclusions
drawn from such models. The objection supplies further evidence that the
weighting is uncertain, which is our thesis rather than an objection to it.

Two smaller points. Our model represents acquired immunity alongside viral
ageing, so we do not attribute to one mechanism what the other does. And our
aim is a model that may be useful in future health emergencies rather than a
retrodiction of past ones. Whether the trade-off dominated in 1918 or in 2022
is a different question from whether it could matter next time.}
